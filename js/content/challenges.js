function setupchallenges(){
  ...
  var c2bonus1 = new LinearEffect(player.quarkstage.upgrades[0], 1, .12, EffectTypes.UpgradeIncreaseMultiplier, null, (obj) => "+" + formatDecimalOverride(obj.increase, 2) + " Acceleron Power Multiplier || Acceleron power *" + formatDecimalOverride(obj.value, 2))
  ...
  player.challenges.push(new Challenge("c2", "[c2] Cutting Down Down Down", "Electrify and reach the goal, but Acceleron and Multor have no effect", new StaticEffect([player.quarkstage.upgrades[0],player.quarkstage.upgrades[5]], 0,EffectTypes.UpgradeIncreaseMultiplier, null, () => "Multors and Accelerons have no effect"), [c2bonus1,c2bonus2,c2bonus3], new ExponentialNumRequirement(player.quarkstage.quarks, "1e20", "1e3"), 100, () => {player.quarkstage.electrify.prestige();}));
}
